...
import dpkt
import re
from statemachine import frameInfoArr, frameShortInfoArr
		...
		for timestamp, buf in pcapng: # for each http2 message
			...
			try:
				...
				for frame in frames: # for each frame in a message
					h2frame_buf = h2.H2Frame(str(frame))
					h2msg.frames.append(h2frame_buf)
				h2msg_arr.append(h2msg)
			except dpkt.http2.HTTP2Exception as e:
				print("  [WARNING] Error in parser_h2_from_pcap(): %s" % e)
				print("    [ ] Error frame: id %d" % frameid)
			frameid += 1
...
def h2msg_to_str(h2msg):
	...
	frameStr = ''
	for frameValue in h2msg.frames:
